...
class AppStore extends Component {
  state = {activateId: tabsList[0].tabId, searchInput: ''}

  getFilterAppList = () => {
    const {activateId} = this.state
    const filterAppList = appsList.filter(
      eachList => eachList.category === activateId,
    )
    return filterAppList
  }

  updateTabId = tabId => {
    this.setState({activateId: tabId})
  }
  ...
  render() {
    const {activateId, searchInput} = this.state
    const filterAppList = this.getFilterAppList()

    const filterSearchResult = filterAppList.filter(eachList =>
        ...
        <div className="app-store">
          ...
          <ul className="tabs-list">
            {tabsList.map(eachTabList => (
              <TabItem
                tabItems={eachTabList}
                key={eachTabList.tabId}
                updateTabId={this.updateTabId}
                isActiveId={activateId === eachTabList.tabId}
              />
            ))}
          </ul>
